            ...
            "2" -> {
                ...
                while (printAndRead("").uppercase() == "Y") {
                    ...
                    try {
                        Files.write(
                            pathToSave,
                            Json.encodeToString(QueryJson(accountName, accountKey, containerName, filesRegex)).encodeToByteArray()
                        )
                    } catch (e: IOException) {
                        print(
                            """
...
fun fromFile(path: String): Query {
    val config = Json.decodeFromString<QueryJson>(  Files.readAllLines(Path(path)).joinToString(separator = " "))
